...
parser.add_argument('--image_path', type=str, default='mmexport1690774465381.jpg', help='需要预测图像的路径')
...
def draw_face(image_path, boxes_c, landmarks):
    ...
    for i in range(boxes_c.shape[0]):
        ...
        cv2.rectangle(img, (corpbbox[0], corpbbox[1]),
                      (corpbbox[2], corpbbox[3]), (0, 0,255), 5)
        # 判别为人脸的置信度
    ...
    for i in range(landmarks.shape[0]):
        for j in range(len(landmarks[i]) // 2):
            cv2.circle(img, (int(landmarks[i][2 * j]), int(int(landmarks[i][2 * j + 1]))), 5, (0, 0, 255))
...
# if __name__ == '__main__':
    ...
    # summary(pnet, input_size=(1, 3, 12, 12))
    # summary(rnet, input_size=(1, 3, 24, 24))
    # summary(onet, input_size=(1, 3, 48, 48))
