...
optimizer = Lion(params=model.parameters(), lr=0.001, weight_decay=1e-4)
...
for epoch in range(epoch_num):
    for batch_id, (img, label, bbox, landmark) in enumerate(train_loader):
        if batch_id==0:
            ...
            print(landmark.shape,"\t", landmark[0])
        img = img.to(device)
        ...
        total_loss = ratio_cls_loss * cls_loss + ratio_bbox_loss * box_loss + ratio_landmark_loss * landmarks_loss
        optimizer.zero_grad()
        total_loss.backward()
        optimizer.step()
        if batch_id % 100 == 0:
