...
class BBox:
    ...
    def __init__(self, box):
        """默认的输入是（xlt, ylt, xrb, yrb）
        涉及的操作是：将bbox内的关键点与bbox之间的转化关系。
        @IPF
        """
...
def IOU(box, boxes, eps=1e-10):
    ...
    # 重叠部分面积
    inter = w * h
    return inter / (box_area + area - inter + eps)  # 防止inf
...
def get_landmark_from_lfw_neg(txt, data_path, with_landmark=True):
    ...
    参数：
      txt：数据txt文件 "wider_face_train_bbx_gt.txt"
      data_path:数据存储目录
...
def crop_landmark_image(data_dir, data_list, size, argument=True, sup_del=40):
    """裁剪并保存带有人脸关键点的图片
    参数：
      data_dir：数据目录
      size:裁剪图片的大小
    ...
    for (imgPath, box, landmarkGt) in tqdm(data_list):
        ...
        if argument:
            ...
            gt_h = y2 - y1 + 1
            # 除去过小图像
            if max(gt_w, gt_h) < sup_del or x1 < 0 or y1 < 0:
                continue
            for i in range(10):
                # 随机裁剪图像大小
